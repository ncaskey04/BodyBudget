		...
		$.when(searchFood(recipe)).done(function(result){
			...
			if(result.matches.length === 0){
				...
				result.matches.forEach(function (itm){
					// $(".user-data").append("<img src="+itm.smallImageUrls+">")
					$.when(searchFoodId(itm.id)).done(function(data){
						...
						$(".user-data").append("<h3>"+data.name+"</h3>")
						//
						$(".user-data").append("<img src="+data.images[0].hostedMediumUrl+">")
						data.nutritionEstimates.forEach(function(nutrition){
							if(nutrition.attribute === "ENERC_KJ"){
								// console.log(nutrition.value)
								$(".user-data").append("<p> Calorie: "+nutrition.value+"</p>")
								$(".user-data").data("Calorie",  nutrition.value)
							}
							if(nutrition.attribute === "FAT"){
								// console.log(nutrition.value)
								$(".user-data").append("<p> Fat: "+nutrition.value+"</p>")
								$(".user-data").data("Fat",  nutrition.value)
							}
							if(nutrition.attribute === "PROCNT"){
								// console.log(nutrition.value)
								$(".user-data").append("<p> Protein: "+nutrition.value+"</p>")
								$(".user-data").data("Protein",  nutrition.value)
							}
							if(nutrition.attribute === "CHOCDF"){
								// console.log(nutrition.value)
								$(".user-data").append("<p> Carbs: "+nutrition.value+"</p>")
								$(".user-data").data("Carbs",  nutrition.value)
							}
						})
							// $(".user-data").data("protein", data.nutritionEstimates)
					})
					
					// recipeResult.forEach(function (food){
					// 	console.log(food.responseJSON)
					// })
					// $(".user-data").append("<p>"+itm.id+"</p>")

					// $(".user-data").data("protein",  itm.protein)
					// $(".user-data").data("fat",  itm.fat)
					// $(".user-data").data("carbs",  itm.carbs)
				})
			}
		});
	});
